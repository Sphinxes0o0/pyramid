---
source: https://book.systemsapproach.org/internetworking/basic-ip.html
title: "3.3 Internet (IP) &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation"
type: ebook-section
---

# 3.3 Internet (IP) &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation

3.3 Internet (IP) — Computer Networks: A Systems Approach Version 6.2-dev documentation
      -
          [
...
[Problem: Not All Networks are Directly Connected](problem.html)
...
- [Print Copies](../print.html)
      ...
      - 3.3 Internet (IP)
      -
            [ View page source](../_sources/internetworking/basic-ip.rst.txt)
        [ Previous](ethernet.html)
        [Next ](routing.html)
# 3.3 Internet (IP)
...
[![../_images/f03-14-9780123850591.png](./images/f03-14-9780123850591.png)
](../_images/f03-14-9780123850591.png)
Figure 70. A simple internetwork. H denotes a host and R denotes
a router.
Figure 70 shows an example internetwork. An
...
[![../_images/f03-15-9780123850591.png](./images/f03-15-9780123850591.png)
](../_images/f03-15-9780123850591.png)
Figure 71. A simple internetwork, showing the protocol layers
...
function as a single logical internetwork. For example, Figure
71 shows how hosts H5 and H8 are logically connected by
the internet in Figure 70, including the protocol graph
...
is shown in Figure 72. Note that we have
...
[![../_images/f03-16-9780123850591.png](./images/f03-16-9780123850591.png)
](../_images/f03-16-9780123850591.png)
Figure 72. IPv4 packet header.
...
[![../_images/f03-17-9780123850591.png](./images/f03-17-9780123850591.png)
](../_images/f03-17-9780123850591.png)
Figure 73. IP datagrams traversing the sequence of physical
networks graphed in the earlier figure.
To see what this all means, consider what happens when host H5 sends a
datagram to host H8 in the example internet shown in Figure
70. Assuming that the MTU is 1500 bytes for the two
...
situation is illustrated in Figure 73. This
...
which it travels.
[![../_images/f03-18-9780123850591.png](./images/f03-18-9780123850591.png)
](../_images/f03-18-9780123850591.png)
Figure 74. Header fields used in IP fragmentation:
(a) unfragmented packet; (b) fragmented packets.
The fragmentation process can be understood in detail by looking at
the header fields of each datagram, as is done in Figure 74.  The unfragmented packet, shown at the top, has
...
bit in the `Flags` field (see Figure 72),
...
three different classes, as shown in Figure 75,
...
each of them can accommodate up to \(2^{24} - 2\) (about
...
However, the addressing scheme supports 221 class C networks.
[![../_images/f03-19-9780123850591.png](./images/f03-19-9780123850591.png)
](../_images/f03-19-9780123850591.png)
Figure 75. IP addresses: (a) class A; (b) class B; (c) class C.
...
if (NetworkNum of destination = my NetworkNum) then
    ...
    deliver packet to default router
Let’s see how this works in the example internetwork of Figure
70. First, suppose that H1 wants to send a datagram to H2.
...
table shown in Table 10, it looks up H8’s
...
Table 10. Forwarding table for Router R2.
NetworkNum
NextHop
1
R1
...
in Table 11.
Table 11. Complete Forwarding table for Router R2.
NetworkNum
NextHop
1
R1
...
addresses, we only need to connect 214 (about 16,000) class B
...
[![../_images/f03-20-9780123850591.png](./images/f03-20-9780123850591.png)
](../_images/f03-20-9780123850591.png)
Figure 76. Subnet addressing.
...
attached.  For example, host H1 in Figure 77 is
...
[![../_images/f03-21-9780123850591.png](./images/f03-21-9780123850591.png)
](../_images/f03-21-9780123850591.png)
Figure 77. An example of subnetting.
...
network of Figure 77, router R1 would have the entries
shown in Table 12.
Table 12. Example Forwarding Table with Subnetting.
SubnetNumber
SubnetMask
NextHop
128.96.34.0
...
outside the campus see the collection of networks in Figure
77 as just the network 128.96, and they keep one entry in
...
always the case).
[
](../_images/f03-22-9780123850591.png)
Figure 78. Route aggregation with CIDR.
...
Consider the example in Figure 78. Assume that eight
...
Figure 79. ARP packet format for mapping IP addresses into Ethernet addresses.
Figure 79 shows the ARP packet format for
...
in a forwarding table like the one in Table 10.
...
message from a host to a remote DHCP server is shown in Figure
80.
[
](../_images/f03-24-9780123850591.png)
Figure 80. A DHCP relay agent receives a broadcast DHCPDISCOVER
message from a host and sends a unicast DHCPDISCOVER to the DHCP
server.
Figure 81 below shows the format of a DHCP
...
DHCP packet.”
[
](../_images/f03-25-9780123850591.png)
Figure 81. DHCP packet format.
...
improves the manageability of a network. [[Next]](routing.html#key-forwarding)
...
corporation Y.
Figure 82(a) shows two private networks for two
separate corporations. In Figure 82(b) they are
...
[
](../_images/f03-26-9780123850591.png)
Figure 82. An example of virtual private networks: (a) two
separate private networks; (b) two virtual private networks
sharing common switches.
In Figure 82, a virtual circuit network (using ATM, for
...
[
](../_images/f03-27-9780123850591.png)
Figure 83. A tunnel through an internetwork. 18.5.0.1 is the
...
example, the network in Figure 83. A tunnel has
...
Table 13.
Table 13. Forwarding Table for Router R1.
NetworkNum
NextHop
1
Interface 0
...
network 2, it forwards the packet on to that network. Figure
83 shows the change in encapsulation of the packet as it
