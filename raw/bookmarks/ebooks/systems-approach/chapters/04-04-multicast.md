---
source: https://book.systemsapproach.org/scaling/multicast.html
title: "4.3 Multicast &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation"
type: ebook-section
---

# 4.3 Multicast &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation

4.3 Multicast — Computer Networks: A Systems Approach Version 6.2-dev documentation
      -
          [
...
- [Print Copies](../print.html)
      ...
      - 4.3 Multicast
      -
            [ View page source](../_sources/scaling/multicast.rst.txt)
        [ Previous](ipv6.html)
        [Next ](mpls.html)
# 4.3 Multicast
...
the high-order 5 bits. Thus, 32 (25) IP addresses map into each
...
[![../_images/f04-14-9780123850591.png](./images/f04-14-9780123850591.png)
](../_images/f04-14-9780123850591.png)
Figure 110. PIM operation: (a) R4 sends a Join message to RP and joins
...
Figure 110(a), in which router R4 is sending
...
the RP to R4 in Figure 110(a).
As more routers send `Join`s toward the RP, they cause new branches
to be added to the tree, as illustrated in Figure
110(b). Note that, in this case, the `Join` only needs
...
Figure 110. There is no state for this
...
example of Figure 110, this means that the
...
Figure 111. We see the tunneled packet
...
a `Join` message toward the sending host (Figure
110(c)). As this `Join` travels toward the host, it
...
[![../_images/f04-15-9780123850591.png](./images/f04-15-9780123850591.png)
](../_images/f04-15-9780123850591.png)
Figure 111. Delivery of a packet along a shared tree. R1 tunnels
...
Figure 110(c), we see a source-specific
...
Figure 110(d). Note that this tree no longer
...
originating RP.
[![../_images/f04-16-9780123850591.png](./images/f04-16-9780123850591.png)
](../_images/f04-16-9780123850591.png)
Figure 112. MSDP operation: (a) The source SR sends a Register message
...
Figure 112(a). The `Join` message builds a
branch of the source-specific tree to this RP, as shown in
Figure 112(b).  The result is that every RP that
...
from R1 to R2 in Figure 113(b) for an
...
terminates. Figure 113(a) shows a `Join`
...
receive the packet. Figure 113(b) illustrates
the flow of multicast traffic originating at R1.
[![../_images/f04-17-9780123850591.png](./images/f04-17-9780123850591.png)
](../_images/f04-17-9780123850591.png)
Figure 113. BIDIR-PIM operation: (a) R2 and R3 send Join messages toward
