---
source: https://book.systemsapproach.org/congestion/tcpcc.html
title: "6.3 TCP Congestion Control &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation"
type: ebook-section
---

# 6.3 TCP Congestion Control &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation
...
[Problem: Allocating Resources](problem.html)
- [6.1 Issues in Resource Allocation](issues.html)
- [6.2 Queuing Disciplines](queuing.html)
- 6.3 TCP Congestion Control
6.3.1 Additive Increase/Multiplicative Decrease
- 6.3.2 Slow Start
- 6.3.3 Fast Retransmit and Fast Recovery
- 6.3.4 TCP CUBIC
- [6.4 Advanced Congestion Control](avoidance.html)
- [6.5 Quality of Service](qos.html)
- [Perspective: Software-Defined Traffic Engineering](trend.html)
...
- [Print Copies](../print.html)
      ...
      - 6.3 TCP Congestion Control
      -
            [ View page source](../_sources/congestion/tcpcc.rst.txt)
        [ Previous](queuing.html)
        [Next ](avoidance.html)
# 6.3 TCP Congestion Control
...
segment size* .
[![../_images/f06-08-9780123850591.png](./images/f06-08-9780123850591.png)
](../_images/f06-08-9780123850591.png)
Figure 159. Packets in transit during additive increase, with one
...
is illustrated in Figure 159. Note that, in
...
[![../_images/f06-09-9780123850591.png](./images/f06-09-9780123850591.png)
](../_images/f06-09-9780123850591.png)
Figure 160. Typical TCP sawtooth pattern.
...
time, you get a sawtooth pattern, as illustrated in Figure 160. The important concept to understand about AIMD is
...
Figure 161 shows the growth in the number
of packets in transit during slow start. Compare this to the linear
growth of additive increase illustrated in Figure 159.
[![../_images/f06-10-9780123850591.png](./images/f06-10-9780123850591.png)
](../_images/f06-10-9780123850591.png)
Figure 161. Packets in transit during slow start.
...
grow.
Figure 162 traces how TCP’s `CongestionWindow`
...
[![../_images/f06-11-9780123850591.png](./images/f06-11-9780123850591.png)
](../_images/f06-11-9780123850591.png)
Figure 162. Behavior of TCP congestion control. Colored line = value
...
about 5.5 seconds:
- A timeout happens, causing the congestion window to be divided by 2,
dropping it from approximately 22 KB to 11 KB, and
`CongestionThreshold` is set to this amount.
- `CongestionWindow` is reset to one packet, as the sender enters
slow start.
- Slow start causes `CongestionWindow` to grow exponentially until it
...
[![../_images/f06-12-9780123850591.png](./images/f06-12-9780123850591.png)
](../_images/f06-12-9780123850591.png)
Figure 163. Fast retransmit based on duplicate ACKs.
Figure 163 illustrates how duplicate ACKs lead
...
[![../_images/f06-13-9780123850591.png](./images/f06-13-9780123850591.png)
](../_images/f06-13-9780123850591.png)
Figure 164. Trace of TCP with fast retransmit. Colored line
...
transmitted.
Figure 164 illustrates the behavior of a version
of TCP with the fast retransmit mechanism. It is interesting to
compare this trace with that given in Figure 162, where fast retransmit was not implemented—the long
...
between 3.8 and 4 seconds in Figure 164 and
...
[![../_images/Slide11.png](./images/Slide11.png)
](../_images/Slide11.png)
Figure 165. Generic cubic function illustrating the change in the congestion
...
example is shown in Figure 165, which we have
...
(denoted \(W_{max}\)). The idea is to start fast but slow the
growth rate as you get close to \(W_{max}\), be cautious and have
near-zero growth when close to \(W_{max}\), and then increase the
growth rate as you move away from \(W_{max}\). The latter phase is
essentially probing for a new achievable \(W_{max}\).
...
C is a scaling constant and \(\beta\) is the multiplicative
decrease factor.  CUBIC sets the latter to 0.7 rather than the 0.5
that standard TCP uses. Looking back at Figure 165, CUBIC is often described as shifting between a concave
