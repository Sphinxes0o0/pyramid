...
title: "5.3 Remote Procedure Call &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation"
type: ebook-section
---

# 5.3 Remote Procedure Call &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation
...
- [Print Copies](../print.html)
      ...
      - 5.3 Remote Procedure Call
      -
            [ View page source](../_sources/e2e/rpc.rst.txt)
        [ Previous](tcp.html)
        [Next ](rtp.html)
# 5.3 Remote Procedure Call
...
(suspending execution) to wait for the reply. Figure
137 illustrates the basic interaction between the
client and server in such an exchange.
[![../_images/f05-13-9780123850591.png](./images/f05-13-9780123850591.png)
...
- A protocol that manages the messages sent between the client and the
server processes and that deals with the potentially undesirable
properties of the underlying network.
- Programming language and compiler support to package the arguments
...
Figure 138 schematically depicts what happens
...
[![../_images/f05-14-9780123850591.png](./images/f05-14-9780123850591.png)
...
given in Figure 139. The client sends a
...
[![../_images/f05-15-9780123850591.png](./images/f05-15-9780123850591.png)
...
mechanism.
[![../_images/f05-16-9780123850591.png](./images/f05-16-9780123850591.png)
](../_images/f05-16-9780123850591.png)
Figure 140. Timeline for a reliable RPC protocol using implicit
acknowledgment.
...
Figure 141 illustrates the protocol graph when
...
[![../_images/f05-17-9780123850591.png](./images/f05-17-9780123850591.png)
...
[![../_images/f05-18-9780123850591.png](./images/f05-18-9780123850591.png)
...
paragraphs.
[![../_images/f05-19-9780123850591.png](./images/f05-19-9780123850591.png)
...
[![../_images/f05-20-9780123850591.png](./images/f05-20-9780123850591.png)
...
(containers) that implement that service, as shown in Figure
145. The load balancer can be implemented in different
...
HTTP response. The full gRPC stack is depicted in Figure
146, which also includes the language-specific elements.
(One strength of gRPC is the wide set of programming languages it
supports, with only a small subset shown in Figure
146.)
[
](../_images/Slide21.png)
Figure 146. gRPC core stacked on top of HTTP, TLS, and TCP and
...
patterns:
- Simple RPC: The client sends a single request message and the server
responds with a single reply message.
- Server Streaming RPC: The client sends a single request message and
the server responds with a stream of reply messages. The client
completes once it has all the server’s responses.
- Client Streaming RPC: The client sends a stream of requests to the
...
- Bidirectional Streaming RPC: The call is initiated by the client, but
...
exchange messages with each other. [[Next]](rtp.html#key-alf)
        [ Previous](tcp.html)
        [Next ](rtp.html)
    © Copyright 2024.
