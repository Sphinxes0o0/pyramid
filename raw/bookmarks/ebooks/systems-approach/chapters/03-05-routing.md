...
title: "3.4 Routing &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation"
type: ebook-section
---

# 3.4 Routing &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation
...
- [Print Copies](../print.html)
      ...
      - 3.4 Routing
      -
            [ View page source](../_sources/internetworking/routing.rst.txt)
        [ Previous](basic-ip.html)
        [Next ](impl.html)
# 3.4 Routing
...
Table 14 gives an example of a row from a routing
...
18/8
171.69.245.10
In contrast, Table 15 gives an example of a row from a
...
## 3.4.1 Network as a Graph
Routing is, in essence, a problem of graph theory. Figure 84 shows a graph representing a network. The nodes of
...
own edge cost.
[![../_images/f03-28-9780123850591.png](./images/f03-28-9780123850591.png)
...
in Figure 84, you could imagine just
...
[![../_images/f03-29-9780123850591.png](./images/f03-29-9780123850591.png)
...
consider an example like the one depicted in Figure 85. In this example, the cost of each link is set to 1, so
...
other nodes as a table like Table 16. Note that
...
We may consider each row in Table 16 as a list
...
to all the nodes is called *convergence*. Table 19 shows the final set of costs from each node to all other
...
Figure 86, router C would advertise to router A
...
at cost 2.
[![../_images/f03-30-9780123850591.png](./images/f03-30-9780123850591.png)
](../_images/f03-30-9780123850591.png)
Figure 86. Example network running RIP.
[![../_images/f03-31-9780123850591.png](./images/f03-31-9780123850591.png)
...
Figure 87. The majority of the packet is taken up
...
[![../_images/f03-32-9780123850591.png](./images/f03-32-9780123850591.png)
](../_images/f03-32-9780123850591.png)
Figure 88. Flooding of link-state packets: (a) LSP arrives at
node X; (b) X floods LSP to A and C; (c) A and C flood LSP to B
(but not X); (d) flooding is complete.
Figure 88 shows an LSP being flooded in a small
network.  Each node becomes shaded as it stores the new LSP. In
Figure 88(a) the LSP arrives at node X, which
sends it to neighbors A and C in Figure 88(b). A
...
- For each neighbor (`Neighbor`) of `Next`, calculate the cost
...
reach `Next`.
- If `Neighbor` is currently on the `Tentative` list, and the
...
- If the `Tentative` list is empty, stop. Otherwise, pick the entry
from the `Tentative` list with the lowest cost, move it to the
`Confirmed` list, and return to step 2.
[![../_images/f03-33-9780123850591.png](./images/f03-33-9780123850591.png)
](../_images/f03-33-9780123850591.png)
Figure 89. Link-state routing: an example network.
This will become a lot easier to understand when we look at an
example.  Consider the network depicted in Figure 89. Note that, unlike our previous example, this network
has a range of different edge costs. Table 20
...
paths to all nodes.
Table 20. Steps for Building Routing Table for Node D.
Step
Confirmed
Tentative
Comments
...
routing in [Section 3.5](impl.html#implementation) when we
introduce Software Defined Networking (SDN). [Next]
### The Open Shortest Path First Protocol (OSPF)
...
network capacity.
[![../_images/f03-34-9780123850591.png](./images/f03-34-9780123850591.png)
](../_images/f03-34-9780123850591.png)
Figure 90. OSPF header format.
There are several different types of OSPF messages, but all begin with
the same header, as shown in Figure 90. The
...
[![../_images/f03-35-9780123850591.png](./images/f03-35-9780123850591.png)
](../_images/f03-35-9780123850591.png)
Figure 91. OSPF link-state advertisement.
Figure 91 shows the packet format for a
...
function that is shown graphically in Figure 92. below. Observe the following:
[
](../_images/f03-36-9780123850591.png)
Figure 92. Revised ARPANET routing metric versus link
utilization.
...
curves in Figure 92 were arrived at by a great
...
possible is usually the best approach.  [[Next]](impl.html#key-control-data)
        [ Previous](basic-ip.html)
        [Next ](impl.html)
    © Copyright 2024.
