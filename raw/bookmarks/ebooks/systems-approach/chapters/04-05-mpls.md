---
source: https://book.systemsapproach.org/scaling/mpls.html
title: "4.4 Multiprotocol Label Switching &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation"
type: ebook-section
---

# 4.4 Multiprotocol Label Switching &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation

4.4 Multiprotocol Label Switching — Computer Networks: A Systems Approach Version 6.2-dev documentation
      -
          [
...
- [Print Copies](../print.html)
      ...
      - 4.4 Multiprotocol Label Switching
      -
            [ View page source](../_sources/scaling/mpls.rst.txt)
        [ Previous](multicast.html)
        [Next ](mobile-ip.html)
# 4.4 Multiprotocol Label Switching
...
this idea works.
[![../_images/f04-18-9780123850591.png](./images/f04-18-9780123850591.png)
](../_images/f04-18-9780123850591.png)
Figure 114. Routing tables in example network.
Consider the network in Figure 114. Each of
...
illustrated in Figure 115. Router R2 has
...
In Figure 115(c), we see another label
...
place in its table.
[![../_images/f04-19-9780123850591.png](./images/f04-19-9780123850591.png)
](../_images/f04-19-9780123850591.png)
Figure 115. (a) R2 allocates labels and advertises bindings to
...
methods for carrying labels on packets are shown in Figure
116. When IP packets are carried as complete frames, as they
...
[![../_images/f04-20-9780123850591.png](./images/f04-20-9780123850591.png)
](../_images/f04-20-9780123850591.png)
Figure 116. (a) Label on an ATM-encapsulated packet; (b) label on
...
same protocols, consider the alternative. In Figure 117(a), we see a set of routers interconnected by virtual
...
[![../_images/f04-21-9780123850591.png](./images/f04-21-9780123850591.png)
](../_images/f04-21-9780123850591.png)
Figure 117. (a) Routers connect to each other using an overlay of
...
adjacencies. By contrast, in Figure 117(b), the
...
inside a service provider’s network. Figure 118
...
the head).
[![../_images/f04-22-9780123850591.png](./images/f04-22-9780123850591.png)
](../_images/f04-22-9780123850591.png)
Figure 118. A network requiring explicit routing.
Suppose that the operator of the network in Figure 118 has determined that any traffic flowing from R1 to R7
...
emulation*. Figure 119 illustrates the idea.
[![../_images/f04-23-9780123850591.png](./images/f04-23-9780123850591.png)
](../_images/f04-23-9780123850591.png)
Figure 119. An ATM circuit is emulated by a tunnel.
...
Looking back to our first example, in Figure 115, we saw that router R1 attached a label (`15`) to
...
across a network of MPLS-enabled routers, as in Figure
119. Further, we assume that the goal is to emulate an
...
ATM cell would be forwarded. Figure 120
illustrates the steps.
- An ATM cell arrives on the designated input port with the appropriate
VCI value (101 in this example).
- The head router attaches the demultiplexing label that identifies the
emulated circuit.
- The head router then attaches a second label, which is the tunnel
...
section.
- Routers between the head and tail forward the packet using only the
tunnel label.
- The tail router removes the tunnel label, finds the demultiplexing
label, and recognizes the emulated circuit.
- The tail router modifies the ATM VCI to the correct value (202 in
this case) and sends it out the correct port.
[![../_images/f04-24-9780123850591.png](./images/f04-24-9780123850591.png)
](../_images/f04-24-9780123850591.png)
Figure 120. Forward ATM cells along a tunnel.
...
[![../_images/f04-25-9780123850591.png](./images/f04-25-9780123850591.png)
](../_images/f04-25-9780123850591.png)
Figure 121. Example of a layer 3 VPN. Customers A and B each
...
Figure 121. As in layer 2 VPNs, MPLS is used
