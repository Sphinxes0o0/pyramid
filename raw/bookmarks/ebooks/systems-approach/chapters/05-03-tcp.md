...
title: "5.2 Reliable Byte Stream (TCP) &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation"
type: ebook-section
---

# 5.2 Reliable Byte Stream (TCP) &mdash; Computer Networks: A Systems Approach Version 6.2-dev documentation
...
- [Print Copies](../print.html)
      ...
      - 5.2 Reliable Byte Stream (TCP)
      -
            [ View page source](../_sources/e2e/tcp.rst.txt)
        [ Previous](udp.html)
        [Next ](rpc.html)
# 5.2 Reliable Byte Stream (TCP)
...
Figure 127, which, for simplicity, shows
data flowing in only one direction. Remember that, in general, a
single TCP connection supports byte streams flowing in both
directions.
[![../_images/f05-03-9780123850591.png](./images/f05-03-9780123850591.png)
](../_images/f05-03-9780123850591.png)
Figure 127. How TCP manages a byte stream.
The packets exchanged between TCP peers in Figure 127 are called *segments*, since each one carries a
segment of the byte stream. Each TCP segment contains the header
schematically depicted in Figure 128. The
relevance of most of these fields will become apparent throughout this
section. For now, we simply introduce them.
[![../_images/f05-04-9780123850591.png](./images/f05-04-9780123850591.png)
](../_images/f05-04-9780123850591.png)
Figure 128. TCP header format.
...
opposite direction, as illustrated in Figure 129. The use of these three fields is described more fully
later in this chapter.
[![../_images/f05-05-9780123850591.png](./images/f05-05-9780123850591.png)
](../_images/f05-05-9780123850591.png)
Figure 129. Simplified illustration (showing only one direction)
...
connection does a passive open.[1]) Only after this connection
...
illustrated by the timeline given in Figure 130.
[![../_images/f05-06-9780123850591.png](./images/f05-06-9780123850591.png)
](../_images/f05-06-9780123850591.png)
Figure 130. Timeline for three-way handshake algorithm.
...
diagram. A copy of this diagram is given in Figure 131.
...
[![../_images/f05-07-9780123850591.png](./images/f05-07-9780123850591.png)
](../_images/f05-07-9780123850591.png)
...
given by the segment format (as illustrated in Figure 128) and by some application programming interface, such
as the socket API, respectively.
Now let’s trace the typical transitions taken through the diagram in
Figure 131. Keep in mind that at each end of the
...
FIN_WAIT_1 \(\rightarrow\) FIN_WAIT_2 \(\rightarrow\) TIME_WAIT \(\rightarrow\) CLOSED.
- The other side closes first:
ESTABLISHED \(\rightarrow\)
CLOSE_WAIT \(\rightarrow\) LAST_ACK \(\rightarrow\) CLOSED.
- Both sides close at the same time:
ESTABLISHED \(\rightarrow\)
FIN_WAIT_1 \(\rightarrow\) CLOSING \(\rightarrow\) TIME_WAIT \(\rightarrow\) CLOSED.
...
situation illustrated in Figure 132. TCP on the
...
[![../_images/f05-08-9780123850591.png](./images/f05-08-9780123850591.png)
](../_images/f05-08-9780123850591.png)
Figure 132. Relationship between TCP send buffer (a) and receive
buffer (b).
...
data, as in Figure 132. Note that bytes to the left of
...
window size: 232 >> 2 × 216. However, this
...
the pipe full.) Table 22 shows how long it takes
...
United States), Table 23 gives the delay × bandwidth
...
syndrome*. Figure 133 helps visualize what
...
everything is good (Figure 133(a)). But,
...
create larger containers, as in Figure 133(b). This scenario was discovered when early
implementations of TCP regularly found themselves filling the network
with tiny segments.
[![../_images/f05-09-9780123850591.png](./images/f05-09-9780123850591.png)
](../_images/f05-09-9780123850591.png)
Figure 133. Silly window syndrome. (a) As long as the sender sends
...
Figure 134, if you assume that the ACK is for
...
[![../_images/f05-10-9780123850591.png](./images/f05-10-9780123850591.png)
](../_images/f05-10-9780123850591.png)
Figure 134. Associating the ACK with (a) original transmission
...
arithmetic. Instead, the whole calculation is scaled by 2n,
with delta selected to be 1/2n. This allows us to do integer
...
6298](https://tools.ietf.org/html/rfc6298). The takeaway is that
...
recede in importance. [[Next]](rpc.html#key-micro-service)
## 5.2.7 Record Boundaries
...
[
](../_images/f05-11-9780123850591.png)
Figure 135. Measured system: Two Linux workstations and a pair of
Gbps Ethernet links.
...
from one machine to the other. Figure 135
...
throughput test are given in Figure 136. The key
...
to process.
[
](../_images/f05-12-9780123850591.png)
Figure 136. Measured throughput using TCP, for various message
sizes.
...
widespread deployment. As discussed further in [Chapter 9](../applications.html#chapter-9-applications), QUIC was motivated in large part by the
...
[Chapter 8](../security.html#chapter-8-network-security)). While a full
...
Security ([Section 8.5](../security/systems.html#example-systems)) would typically
...
[Chapter 6](../congestion.html#chapter-6-congestion-control)). Specifically, each
...
firewalls (see [Section 8.5](../security/systems.html#example-systems)) have enough
...
transport protocols that we return to in [Chapter 6](../congestion.html#chapter-6-congestion-control).
HTTP went through a number of versions (1.0, 1.1, 2.0, discussed in
[Section 9.1](../applications/traditional.html#traditional-applications)) in an effort to map its
