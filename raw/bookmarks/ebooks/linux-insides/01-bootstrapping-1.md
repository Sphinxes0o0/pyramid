...
上面的跳转指令（ [opcode](http://ref.x86asm.net/coder32.html#xE9) \- 0xe9）跳转到地址 `_start16bit - ( . + 2)` 去执行代码。 `reset` 段是 `16` 字节代码段， 起始于地址 `0xfffffff0`(`src/cpu/x86/16bit/reset16.ld`)，因此 CPU 复位之后，就会跳到这个地址来执行相应的代码 ：
