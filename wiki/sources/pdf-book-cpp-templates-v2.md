---
type: source
source-type: pdf-book
title: C++ 模板（第二版）
author: David Vandevoorde, Nicolai M. Josuttis, Douglas Gregor
date: 2019
size: medium
path: raw/PDFs/books/C++模板_第二版.pdf
pages: 377
summary: 五车书馆翻译版 C++ Templates 第二版：modern C++ 模板完整指南，对应第一版翻译的精简更新，覆盖 C++11 及更新标准。
created: 2019
tags: [cpp]
---
# C++ 模板（第二版）

## Metadata
- **Authors:** David Vandevoorde, Nicolai M. Josuttis, Douglas Gregor
- **Translator:** 五车书馆
- **Pages:** 377
- **Topic:** C++ templates
- **Edition:** 2nd

## Core Content
五车书馆翻译版 C++ Templates 第二版（377页精简版）：modern C++ 模板完整指南，覆盖 C++11 及更新标准（相比第一版删除过时内容，增加 modern C++ 模板新特性）。

## Related Pages
- [[sources/pdf-book-cpp-templates-complete-guide]] — C++ Templates Complete Guide (陈晓伟译版)
- [[sources/pdf-cpp17]] — C++17 guide
- [[sources/pdf-book-modern-cpp]] — Modern C++ books