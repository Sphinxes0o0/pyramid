...
> Ingested from `books/21st Century C, 2nd Edition.pdf` via `lit parse` on 2026-06-04.
> Source file: 5.69 MB.

## Page 1

21st Century C                                                                                                        SECOND
Throw out your old ideas about C and get to know a programming language     “Is your C development                    EDITION
that has substantially outgrown its origins. With this revised edition of 21st       environment limited to vi
Century C, you’ll discover up-to-date techniques missing from other C                and cc? Does C memory
tutorials, whether you’re new to the language or just getting reacquainted.
C isn’t just the foundation of modern programming languages; it is a                 management still plague
modern language, ideal for writing efficient, state-of-the-art applications.         you as it did in the '90s?
Get past idioms that made sense on mainframes and learn the tools you                Ben Klemens capably                         Century
need to work with this evolved and aggressively simple language. No                  addresses these and
matter what programming language you currently favor, you’ll quickly see                 2
                                                                                         1
                                                                                         s
                                                                                         t
                                                                                         C
                                                                                         ...
                                                                                         r
                                                                                         y
that 21st century C rocks.                                                           other common problems,     C
                                                                                     demonstrating how tools
■  Set up a C programming environment with shell facilities,                         have emerged to make
   makefiles, text editors, debuggers, and memory checkers                           C programming easier—
■  Use Autotools, C’s de facto cross-platform package manager                        helping you debug, track
■  Learn about the problematic C concepts too useful to discard                      down memory leaks,
■  Solve C’s string-building problems with C-standard functions                      organize the compilation
■  Use modern syntactic features for functions that take                             process, and manage
   structured inputs                                                                 source code versions
                                                                                         .”
■  Build high-level, object-based libraries and programs                                               —Dave Kitabjian
■  Perform advanced math, talk to internet servers, and run                          Director of Software Development,
   databases with existing C libraries                                                              NetCarrier Telecom

This edition also includes new material on concurrent threads, virtual
tables, C99 numeric types, and other features.                                                                               C TIPS FROM THE
Ben Klemens has written statistical analyses and computationally intensive                                                   NEW SCHOOL
models for Brookings Institution, World Bank, National Institute of Mental
...
PROGRAMMING/C          Twitter: @oreillymedia
...
Editors: Rachel Roumeliotis and Allyson MacDonald     Indexer: Judy McConville
Production Editor: Nicole Shelby                      Interior Designer: David Futato
Copyeditor: Becca Freed                               Cover Designer: Karen Montgomery
...
Coverage  54
Error Checking  55
What is the Userâs Involvement in the Error?  56
...
Use the Shellâs for Loops to Operate on a Set of Files  68
Test for Files  70
fc  73
...
C11Self-conscious about the accusations of selling out, the ISO made few serious
...
## Page 18
    ...
    BSD After Unix was made available from Bell Labs for the public to dissect, the
...
don’t have the bandwidth (or maybe even the disk space) to install every package on
    ...
    2 | Chapter 1: Set Yourself Up for Easy Compilation
...
4 | Chapter 1: Set Yourself Up for Easy Compilation
...
6 | Chapter 1: Set Yourself Up for Easy Compilation
...
8 | Chapter 1: Set Yourself Up for Easy Compilation
...
10 | Chapter 1: Set Yourself Up for Easy Compilation
...
12 | Chapter 1: Set Yourself Up for Easy Compilation
...
14 | Chapter 1: Set Yourself Up for Easy Compilation
...
PANTS=kakhi env | grep PANTS
    ...
    16 | Chapter 1: Set Yourself Up for Easy Compilation
...
gram name, darn it—but the oddness aside, you can see that it took little setup within
   ...
   When your program is a little further along, you can take the time to set up the
    ...
    18   | Chapter 1: Set Yourself Up for Easy Compilation
...
20 | Chapter 1: Set Yourself Up for Easy Compilation
...
22 | Chapter 1: Set Yourself Up for Easy Compilation
...
net search engine can probably offer several (and in the case of strtok, see the section
    ...
    24 | Chapter 1: Set Yourself Up for Easy Compilation
...
26  | Chapter 1: Set Yourself Up for Easy Compilation
...
28 | Chapter 1: Set Yourself Up for Easy Compilation
...
4 There is a POSIX custom that if the first line of file is #!aninterpreter, then when you run file from the
    ...
    GitHub.
        ...
        Compiling C Programs via Here Document     |     29
...
run debug sessions via sudo(!), like sudo lldb stddev_bugged.
    ...
    32 | Chapter 2: Debug, Test, Document
...
34   |     Chapter 2: Debug, Test, Document
...
36 | Chapter 2: Debug, Test, Document
...
38 | Chapter 2: Debug, Test, Document
...
40   |     Chapter 2: Debug, Test, Document
...
Go           run                           Run the program from the start.
             ...
             run args                      Run the program from the start, with the given command-line
                                           arguments.

Stop         b get_rss                     Pause your program at a certain function.

             b nyt_feeds.c:105             Pause just before a certain line of code.

             break 105                     Same as b nyt_feeds.c:105 if you are already stopped in
                                           nyt_feeds.c.

             info break [GDB]              List breakpoints.

             break list [LLDB]

             watch curl [GDB]              Break if the value of the given variable changes.

             watch set var curl [LLDB]

             dis 3 / ena 3 / del 3 [GDB]   Disable/reenable/delete breakpoint 3. If you have a lot of breakpoints
...
Inspect      p url                         Print the value of url. You may specify any expression, including
variables                                  function calls.

             p *an_array@10 [GDB]          Print the first 10 elements of an_array. The next 10 are p
                                           *(an_array+10)@10.

             mem read -tdouble -c10        Read a count of 10 items of type double from an_array. The next
             an_array                      10 are mem read -tdouble -c10 an_array+10.

             info args / info vars         Get the values of all arguments to the function or all local variables.
             [GDB]

             frame var [LLDB]              Get the values of all arguments to the function and all local variables.

             disp url                      Display the value of url every time the program stops.
...
Group          Command               Meaning
               undisp 3              Stop the display the of display item 3. GDB: with no number, turn them
                                     all off.

Threads        info thread [GDB]     List the active threads.

               thread list [LLDB]

               thread 2 [GDB]        Switch focus to thread 2
...
Step           s                     Step one line, even if that means entering another function.
               ...
               n                     Next line, but do not enter subfunctions, and possibly back up to the
                                     head of a loop.

               u                     Until the next line forward from the current line (so let an already-
                                     visited loop run through until forward progress).

               c                     Continue until the next breakpoint or the end of the program.

               ret or ret 3 [GDB]    Return from the current function immediately with the given return
                                     value (if any).

               j 105 [GDB]           Jump to whatever line you please (within reason).

Look at code   l                     list prints the 10 lines around the line you are currently on.

Repeat         Enter                 Just hitting Enter will repeat the last command, which makes stepping
                                     easier, or after l, Enter will list the next 10 lines after those you just
                                     saw.

Compile        make [GDB]            Run make without exiting GDB. You can also specify a target, like
                                     make myprog.

Get help       help                  Explore everything else the debugger offers.





42 | Chapter 2: Debug, Test, Document
...
44 | Chapter 2: Debug, Test, Document
...
46 | Chapter 2: Debug, Test, Document
...
need to profile a library as it runs a test program, you’ll have to copy all of the library
  ...
  a new variable that has nothing to do with the count you meant to set; with C, it fails
   ...
   of a 9-element array and then trundle along for a long time before you find out that
    ...
    48 | Chapter 2: Debug, Test, Document
...
15 of sadstrings.c, which are indeed the two calls to free(cmd) in my modified code.
              ...
              at that point.
                  ...
                  Using Valgrind to Check for Errors     |     49
...
50 | Chapter 2: Debug, Test, Document
...
int main(int argc, char **argv){
 ...
 some  extra      assertion  macros,  like  the     string  comparison          macro,
 g_assert_compstr, used here.


52 | Chapter 2: Debug, Test, Document
...
54 | Chapter 2: Debug, Test, Document
...
56   | Chapter 2: Debug, Test, Document
...
58   | Chapter 2: Debug, Test, Document
...
60   | Chapter 2: Debug, Test, Document
...
62 | Chapter 2: Debug, Test, Document
...
66 | Chapter 3: Packaging Your Project
...
• Scope is awkward—pretty much everything is global.
    ...
    68 | Chapter 3: Packaging Your Project
...
70 | Chapter 3: Packaging Your Project
...
Try a Multiplexer
    ...
    72 | Chapter 3: Packaging Your Project
...
74 | Chapter 3: Packaging Your Project
...
• If you want the shell, not make, to do the substitution, then drop the parens and
  ...
  76   | Chapter 3: Packaging Your Project
...
78 | Chapter 3: Packaging Your Project
...
80 | Chapter 3: Packaging Your Project
...
your package as a tarball, and untars it via tar xvzf your_pkg.tgz, which produces a
        ...
        Packaging Your Code with Autotools | 81
...
bin Install to wherever programs go on the system, e.g., /usr/bin or /usr/local/bin.
include
     Install to wherever headers go, e.g., /usr/local/include.
lib Install to wherever libraries go, e.g., /usr/local/lib.



    82 | Chapter 3: Packaging Your Project
...
know what source files are being compiled. Thus, for every item on the right side of
    ...
    Packaging Your Code with Autotools     |                                                 83
...
84   | Chapter 3: Packaging Your Project
...
dict_test_LDADD=libdict.la
    ...
    Packaging Your Code with Autotools     |                                            85
...
86 | Chapter 3: Packaging Your Project
...
parentheses.
    ...
    Packaging Your Code with Autotools | 87
...
88 | Chapter 3: Packaging Your Project
...
Packaging Your Code with Autotools     |     89
...
It is worth testing that your project can be built in a different directory now and then.
 ...
 run, you can throw in any arbitrary shell code you’d like. Before you do, double-check
  ...
  nice, and there’s no need for a macro, because all you need is echo. Here’s a sample
    ...
    90   | Chapter 3: Packaging Your Project
...
Packaging Your Code with Autotools | 91
...
108 | Chapter 5: Playing Nice with Others
...
110 | Chapter 5: Playing Nice with Others
...
112   | Chapter 5: Playing Nice with Others
...
114 | Chapter 5: Playing Nice with Others
...
116 | Chapter 5: Playing Nice with Others
...
118   |   Chapter 5: Playing Nice with Others
...
120 | Chapter 5: Playing Nice with Others
...
40) or named, optional elements like new_person
...
124 | Chapter 6: Your Pal the Pointer
...
variables allocated and destroyed in a stack of frames, and the description of what it
    ...
    126 | Chapter 6: Your Pal the Pointer
...
complex declarations from right to left (see “Noun-Adjective Form” on page 178).
    ...
    Automatic, Static, and Manual Memory | 127
...
memory, auto pointers to static values, et cetera. If you don’t have anything immedi‐
    ...
    128 | Chapter 6: Your Pal the Pointer
...
130 | Chapter 6: Your Pal the Pointer
...
132 | Chapter 6: Your Pal the Pointer
...
134   | Chapter 6: Your Pal the Pointer
...
136 | Chapter 6: Your Pal the Pointer
...
hit a zero-valued element. Let us say (and this is a bad idea) that we’d like to be able to
 use this function for any type of list where a zero value makes sense, so it will take in
  ...
  Everybody has a different view of the world of code, but I rarely see this form outside
    ...
    138 | Chapter 6: Your Pal the Pointer
...
140 | Chapter 6: Your Pal the Pointer
...
142 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
144 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
146 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
148 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
for (int i=0; i< vector_size; i++){
 ...
 sum += vector[i];
     ...
     Labels, gotos, switches, and breaks     |     149
...
150 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
int main(){
 ...
 Labels, gotos, switches, and breaks | 151
...
152 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
154 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
156 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
int, take three arguments: the string to convert, the pointer-to-end, and a base. The
 ...
 Safely Parse Strings to Numbers | 157
...
158 | Chapter 7: Inessential C Syntax that Textbooks Spend a Lot of Time Covering
...
if (cc[i] != cr[i]) return 0;
    ...
    method, NaN boxing, is left as an exercise for the reader.
     ...
     Safely Parse Strings to Numbers | 159
...
#define double(x) 2*x        Needs more parens.
    ...
    162   | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
164 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
when using the -Wall compiler flag, and so is also not transparent to users.
      ...
      Cultivate Robust and Flourishing Macros     |     165
...
166 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
for each list you tell it to care about. It’ll even make sure every list has a terminating
  ...
  That is, this macro is total overkill, and I don’t recommend it for immediate use, but
   it does demonstrate how you can generate lots of little temp variables that follow a
    ...
    Cultivate Robust and Flourishing Macros | 167
...
168 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
consisting of no preprocessing tokens.”
                 ...
                 Cultivate Robust and Flourishing Macros |     169
...
Macro              Meaning
_POSIX_C_SOURCE    Conforms with IEEE 1003.1, aka ISO/IEC 9945. Usually set to a revision date.

_WINDOWS           A Windows box, with the windows.h header and everything defined therein.

__MACOSX__         A Mac running OS X.

__STDC_HOSTED__    The program is being compiled for a computer with an operating system that will call main.

__STDC_IEC_559__   Conforms to IEEE 754, the floating-point standard that eventually became ISO/IEC/IEEE 60559.
                   Notably, the processor can represent NaN, INFINITY, and -INFINITY.

__STDC_VERSION__   The version of the standard the compiler implements: many use 199409L for C89 (as fixed in a
...
__STDC_NO_COMPLEX__ Set to 1 if the implementation does not support complex types.



170 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
Macro             Meaning
__STDC_NO_VLA__   Set to 1 if the implementation does not support variable-length arrays.
...
static_assert(0, "HAVE_ASPRINTF undefined. I still refuse to "
                  ...
                  Cultivate Robust and Flourishing Macros | 171
...
typedef struct {
    ...
    172 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
comment out a block of code. Unlike comments via /* … */, this style of comment‐
    ...
    #if 0
            ...
            #if 0/* code that was already ignored */
            #endif
            ...
    #endif
    But if the nesting is not correct, like
            ...
            ...
                ...
                Cultivate Robust and Flourishing Macros     |     173
...
174   |   Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
able with the extern keyword. E.g., extern int x.
 ...
 Linkage with static and extern | 175
...
176 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
numeric routines. But when I wrote up the tests here in the present day, the difference
    ...
    178 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
compiling, and that means gcc threw every optimization trick it knew at my sample
  programs, and those optimizations knew when to treat pointers as restrict and
    ...
    int main(){
         int a[10];
         int const *b = a;     //...so add a type-cast to the call.
         set_elmt(a, (int*)b);
    }
...
180 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
182 | Chapter 8: Important C Syntax that Textbooks Often Do Not Cover
...
186 | Chapter 9: Easier Text Handling
...
Making String Handling Less Painful with asprintf | 187
...
188 | Chapter 9: Easier Text Handling
...
Stopif(asprintf(&str, "%s", user_input)==-1, return -1, "asprintf failed.")
    ...
    Making String Handling Less Painful with asprintf | 189
...
190 | Chapter 9: Easier Text Handling
...
Making String Handling Less Painful with asprintf | 191
...
192 | Chapter 9: Easier Text Handling
...
194 | Chapter 9: Easier Text Handling
...
isn't useful for any other purpose. */
    ...
    196 | Chapter 9: Easier Text Handling
...
spot on the line of text, but the Hebrew בּ = b, for instance, can be written as a combi‐
nation of ב) U+05D1) and ּ (U+05BC). Vowels are added to the consonant to further
build the character: ָּב = ba (U+05D1 and U+05BC and U+05B8). And how many
...
198   | Chapter 9: Easier Text Handling
...
200 | Chapter 9: Easier Text Handling
...
202 | Chapter 9: Easier Text Handling
...
• Run xgettext to produce an index of strings that need translating, in the form of
    ...
    204 | Chapter 9: Easier Text Handling
...
both amortization(.amount=200000, .rate=4.5, .years=30) and amortiza
    ...
    206 | Chapter 10: Better Structures
...
"but it should be between zero and one.", x);
    ...
    208 | Chapter 10: Better Structures
...
210 | Chapter 10: Better Structures
...
212 | Chapter 10: Better Structures
...
214 | Chapter 10: Better Structures
...
tializers. I mean this. The old school way, without markers for which initializer went
    ...
    216 | Chapter 10: Better Structures
...
convention.
    ...
    *a; you can actually tell what a is.” The natural response to this is that having a longer
    ...
    218   | Chapter 10: Better Structures
...
: !strcasecmp(papertype, "Legal")        ? (size_s) {.width=216, .height=356}
    ...
    220 | Chapter 10: Better Structures
...
Return Multiple Items from a Function | 221
...
222 | Chapter 10: Better Structures
...
features.3
    ...
    224 | Chapter 10: Better Structures
...
226 | Chapter 10: Better Structures
...
The rule is that if an item is initialized multiple times, then the last initialization takes
  ...
  from day one: it is easy to get carried away and build functions with literally dozens of
    ...
    228 | Chapter 10: Better Structures
...
230 | Chapter 10: Better Structures
...
232 | Chapter 10: Better Structures
...
234 | Chapter 10: Better Structures
...
can’t prevent.
    ...
    Inputs are designated initializers, and may include:
        ...
        The Void Pointer and the Structures It Points To     |     235
...
236 | Chapter 10: Better Structures
...
int main(int argc, char **argv){
    ...
    priate inputs.
     ...
     The Void Pointer and the Structures It Points To | 237
...
238 | Chapter 10: Better Structures
...
The Void Pointer and the Structures It Points To | 239
...
240 | Chapter 10: Better Structures
...
#include <glib.h>
                                 ...
                                 The Void Pointer and the Structures It Points To | 241
...
242 | Chapter 10: Better Structures
...
pound literal; if the function has to copy the structure around (as in the recursion),
 then all you need is an equals sign; returning a blank structure is a trivial case of
               ...
               The Void Pointer and the Structures It Points To     |              243
...
for the investment of writing up a quick type definition.
    ...
    244 | Chapter 10: Better Structures
...
246 | Chapter 11: Object-Oriented Programming in C
...
248 | Chapter 11: Object-Oriented Programming in C
...
250 | Chapter 11: Object-Oriented Programming in C
...
dictionary *dictionary_copy(dictionary *in){
 ...
 Extending Structures and Dictionaries | 251
...
252 | Chapter 11: Object-Oriented Programming in C
...
show you a strong and weak form; gcc and clang allow the strong form using the the
                                             ...
                                             Extending Structures and Dictionaries | 253
...
254 | Chapter 11: Object-Oriented Programming in C
...
int main(){
 ...
 This is an anonymous structure.
        ...
        Extending Structures and Dictionaries | 255
...
256   |   Chapter 11: Object-Oriented Programming in C
...
form and has different compilation notes, but is otherwise identical to seam
    ...
    newUse designated initializers on the first line where you need a struct. As an added
       ...
       able to users without an initial call to a setup function.
                     ...
                     Extending Structures and Dictionaries     |     257
...
258 | Chapter 11: Object-Oriented Programming in C
...
260 | Chapter 11: Object-Oriented Programming in C
...
262 | Chapter 11: Object-Oriented Programming in C
...
264 | Chapter 11: Object-Oriented Programming in C
...
266 | Chapter 11: Object-Oriented Programming in C
...
268 | Chapter 11: Object-Oriented Programming in C
...
270 | Chapter 11: Object-Oriented Programming in C
...
272 | Chapter 11: Object-Oriented Programming in C
...
274 | Chapter 11: Object-Oriented Programming in C
...
276 | Chapter 11: Object-Oriented Programming in C
...
278  | Chapter 11: Object-Oriented Programming in C
...
280 | Chapter 11: Object-Oriented Programming in C
...
group_s *group_new(gsl_vector *position){
    ...
    *out = (group_s) {.position=apop_vector_copy(position), .id=id++, .size=1};
    ...
    282 | Chapter 11: Object-Oriented Programming in C
...
284 | Chapter 11: Object-Oriented Programming in C
...
286 |  Chapter 11: Object-Oriented Programming in C
...
290 | Chapter 12: Parallel Threads
...
292 | Chapter 12: Parallel Threads
...
294 | Chapter 12: Parallel Threads
...
296  | Chapter 12: Parallel Threads
...
298   | Chapter 12: Parallel Threads
...
300 | Chapter 12: Parallel Threads
...
302 | Chapter 12: Parallel Threads
...
304 | Chapter 12: Parallel Threads
...
306 | Chapter 12: Parallel Threads
...
308 | Chapter 12: Parallel Threads
...
310 | Chapter 12: Parallel Threads
...
312 | Chapter 12: Parallel Threads
...
preaddition     value; atomic_fetch_sub   subtracts     (but     there is           no
...
314 | Chapter 12: Parallel Threads
...
dure for working with them:
 ...
 3. Copy the  modified         private  copy back   to the    atomic            struct:
...
if ((candidates/test) has no remainder)
    ...
    316 | Chapter 12: Parallel Threads
...
318 | Chapter 12: Parallel Threads
...
long double distance(apop_data *data, apop_model *model){
        ...
        The GNU Scientific Library | 331
...
alignment                                                    Boolean
         A requirement that data elements begin at              True/false. Named after George Boole, an
         certain boundaries in memory. For exam‐                English mathematician living in the early-
         ple, given an 8-bit alignment requirement,             to-mid 1800s.
         a struct holding a 1-bit char followed by           BSD
         an 8-bit int might need 7 bits of padding              Berkeley Software Distribution. An imple‐
         after the char so that the int starts on an            mentation of POSIX.
         8-bit boundary.
ASCII                                                        callback function
         American Standard Code for Information                 A function (A) that is sent as an input to
         Interchange. A standard mapping from                   another function (B) so that function B
         the naïve English character set to the num‐            can call function A over the course of its
         bers 0–127. Tip: on many systems, man                  operation. For example, generalized sort
         ascii will print the table of codes.                   functions typically take as input a function
                                                                to compare two elements.
automatic allocation                                         call graph
         For an automatically allocated variable, its           A box-and-arrow diagram showing which
         space in memory is allocated by the system             functions call and are called by which
         at the point of the variable’s declaration,            other functions.
         then removed at the end of the given
         scope.                                              cetology
Autotools                                                       The study of whales.
         A set of programs from GNU that simplify            compiler
         automatic compilation on any system,                   Formally, the program that converts the
         including      Autoconf,  Automake,          and       (human-readable) text of a program into
         Libtool.                                               (human-illegible)  machine instructions.
Benford’s law                                                   Often used to refer to the preprocessor +
         Leading digits in a wide range of data sets            compiler + linker.
         tend to have a log-like distribution: 1 has         debugger
         about 30% frequency, 2 about 17.5%, … 9                A program for interactive execution of a
         about 4.5%.                                            compiled program, allowing users to pause
...
deep copy
          ...
          values, et cetera. Often useful for under‐         GNU
          standing bugs.                                            Gnu’s Not Unix.
deep copy                                                    GSL
          A copy of a structure containing pointers,                GNU Scientific Library.
          which follows all pointers and makes               heap
          copies of the pointed-to data.                            The space of memory from which man‐
encoding                                                            ually allocated memory is taken. Compare
          The means by which human-language                         with the stack.
          characters are converted into numeric
          codes for processing by the computer. See          IDE
          also ASCII, multibyte encoding, and wide-                 Integrated   development    environment.
          character encoding.                                       Typically a program with a graphical inter‐
                                                                    ties for compilation, debugging, and other
environment variable                                                face based around a text editor, with facili‐
          A variable present in the environment of a                programmer-friendly features.
          program, set by the parent program (typi‐          integration test
          cally the shell).                                         A test that executes a sequence of steps
external pointer                                                    that involve several segments of a code
          See opaque pointer.                                       base (each of which should have its own
floating point                                                      unit test).
          A representation of a number akin to sci‐          library
          entific notation, like 2.3×10^4, with an                  Basically, a program that has no main func‐
          exponent part (in this example, 4) and a                  tion, and is therefore a set of functions,
          mantissa (here, 2.3). After writing down                  typedefs, and variables available for use by
          the mantissa, think of the exponent allow‐                other programs.
          ing the decimal point to float to its correct
          position.                                          linker
                                                                    portions of a program (such as separate
frame                                                               The program that joins together disparate
          The space in the stack in which function                  object files and libraries) and thus recon‐
          information (such as inputs and automatic                 ciles references to external-to-one-object-
          variables) is stored.                                     file functions or variables.
GDB       GNU debugger.                                      Linux  Technically, an operating system kernel,
global                                                              but generally used to refer to a full suite of
          A variable is global when its scope is the                BSD/GNU/Internet Systems Consortium/
          entire program. C doesn’t really have                     Linux/Xorg/… utilities bundled as a uni‐
          global scope, but if a variable is in a header            fied package.
          that can reasonably be expected to be              macro
          included in every code file in a program,                 A (typically) short blob of text for which a
          then it is reasonable to call it a global vari‐           (typically) longer blob is substituted.
          able.                                              manual allocation
glyph                                                               Allocation of a variable on the heap at the
          A symbol used for written communica‐                      programmer’s request, using malloc or
          tion.                                                     calloc, and freed at the user’s request via
                                                                    free.
...
multibyte encoding                                              such as #include and #define. In practice,
          An encoding of text that uses a variable              typically a part of the compiler.
          number of chars to represent a single
          human-language character. Contrast with      process
          wide-character encoding.                              A running program.
mutex         profiler
          Short for mutual exclusion, a structure               A program that reports where your pro‐
          that can be used to ensure that only one              gram is spending its time, so you know
          thread is using a resource at a time.                 where to focus your efforts at speedup.
NaN           pthread
          Not-a-Number. The IEEE 754 (floating-                 POSIX thread. A thread generated using
          point) standard defines this as the out‐              the C threading interface defined in the
          come of mathematical impossibilities like             POSIX standard.
          0/0 or log(-1). Often used as a flag for      RNG
          missing or bad data.                                  Random number generator, where random
object                                                          basically means that one can reasonably
          A data structure and the associated func‐             expect that a sequence of random numbers
          tions that act on the data structure. Ideally,        is not systematically related to any other
          the object encapsulates a concept, provid‐            sequence.
          ing a limited set of entry points for other      RTFM
          code to interact with the object.                     Read the manual.
object file        Sapir-Whorf Hypothesis
          A file      containing      machine-readable          The claim that the language we speak
          instructions. Typically the result of run‐            determines the thoughts we are capable of
          ning a compiler on a source code file.                having. Its weakest form, that we often
opaque pointer                                                  think in words, is obvious; its strongest
          A pointer to data in a format that can’t be           form, that we are incapable of thoughts
          read by the function handling the pointer,            that our language lacks words or construc‐
          but that can be passed on to other func‐              tions for, is clearly false.
          tions that can read the data. A function in      scope
          a scripting language might call one C func‐           The portion of the code base over which a
          tion that returns an opaque pointer to C-             variable is declared and accessible. Good
          side data, and then a later function in the           coding style depends on keeping the scope
          scripting language would use that pointer             of variables small.
          to act on the same C-side data.        segfault
POSIX     The Portable Operating System Interface.              Segmentation fault.
          An IEEE standard to which UNIX-like      segmentation fault
          operating systems conform, describing a               Touching an incorrect segment of mem‐
          set of C functions, the shell, and some               ory. Causes the operating system to halt
          basic utilities.                                      the program immediately, and so often
preprocessor                                                    used metonymically to refer to         any
          Conceptually, a program that runs just                program-halting error.
          before the compiler, executing directives      SHA    Secure Hash Algorithm.
...
shell                                                            variable as data of the second type. For
          A program that allows users to interact                example, given struct    {int a;         char
          with an operating system, either at a com‐             *b:} astruct, then (int) astruct is an
          mand line or via scripts.                              integer (but for a safer alternative, see “C,
SQL                                                              with fewer seams” on page 253). Fre‐
          Structured Query Language. A standar‐                  quently not portable; always bad form.
          dized means of interacting with databases.        type qualifier
stack                                                            A descriptor of how the compiler may
          The space in memory where function exe‐                handle a variable. Is unrelated to the type
          cution occurs. Notably, automatic vari‐                of the variable (int, float, et cetera). C’s
          ables are placed here. Each function gets a            only type qualifiers are const, restrict,
          frame, and every time a subfunction is                 volatile, and _Atomic.
          called, its frame is conceptually stacked on      union
          top of the calling function’s frame.                   A single block of memory that can be
static allocation                                                interpreted as one of several types.
          The method by which variables with file           unit test
          scope and variables      inside  functions             A block of code to test a small piece of a
          declared as static are allocated. Alloca‐              code base. Compare with integration test.
          tion occurs before the program starts, and
          the variable continues to exist until the         UI
          end of the program.                                    User interface.   For a C library, this
test harness                                                     includes the typedefs, macro definitions,
          A system for running a sequence of unit                and function declarations that users are
          tests and integration tests. Provides easy             expected to be comfortable with when
          setup and teardown of auxiliary structures,            using the library.
          and allows for checking of failures that          UTF
          may (correctly) halt the main program.                 Unicode Transformation Format.
thread                                                      variadic function
          A sequence of instructions that a com‐                 A function that takes in a variable number
          puter executes independently of any other              of inputs (e.g., printf).
          thread.                                           wide-character encoding
token                                                            An encoding of text where each human-
          A set of characters to be treated as a                 language character is given a fixed number
          semantic unit, such as a variable name, a              of chars. For example, UTF-32 guarantees
          keyword, or an operator like * or +. The               that each Unicode character is expressed
          first step in parsing text is to break it down         in exactly 4 bytes. Contrast this definition
          into tokens; strtok_r and strtok_n are                 with multibyte encoding.
          designed for this.                                XML
type punning                                                     Extensible Markup Language.
...
Symbols                                A
# (octothorp), 166                     aliases vs. copies, 129
#define, 294                           alignment, 137, 363
#endif, 173                            American National Standards Institute (ANSI),
#if 0, 173                              13
#ifdef, 173                            ANSI C89 standard, 13
#ifndef, 173                           arguments, in macros, 169
#include, 9                            arrays
#pragma, 294                            creating, 348
#pragma once, 173                       designated initializers, 214
$*, 18                                  initialization via compound literals, 207
$<, 18                                  initializing with zeros, 216
$@, 18                                  memory management and, 131
--pedantic flag, 176                    multidimensional, 138
-g flag, 32                             notating, 136
-liberty flag, 185                      setting size at runtime, 144
. (dot) (see source shell command)     ASCII (American Standard Code for Informa‐
_Bool type, 183                         tion Interchange), 197, 363
_Exit function, 150                    asprintf function
_Generic keyword, 271                   constant string, 189
_POSIX_C_SOURCE macro, 170              extending strings with, 191
_Pragma, 294                            sensitive information and, 188
_Thread_local keyword, 129, 299         string allocation with, 185
_WINDOWS macro, 170                    atof function, 156
__attribute__ , 224                    atoi function, 156
__MACOSX__ macro, 170                  atomic structs, 315
__STDC_HOSTED__ macro, 170             atomic variables, 302, 312
__STDC_IEC_559__ macro, 170            atoms, 305
__STDC_NO_ATOMICS__ macro, 170         at_exit function, 150
__STDC_NO_COMPLEX__ macro, 170         at_quick_exit function, 150
__STDC_NO_THREADS__ macro, 171         autocasting, 145
__STDC_NO_VLA__ macro, 171             Autoconf, macro generation by, 171
__STDC_VERSION__ macro, 170            Automake, conditional subdirectory for, 116
__VA_ARGS__ keyword, 208                (see also Libtool)
...
automatic allocation, 363                    Basic Linear Algebra Subprograms (BLAS)
automatic memory management, 124              library, 13
Autotools                                    basic regular expressions (BREs), 323
     contingency hooks in, 117               Benford’s law, 363
     Distutils, 118                          blank tokens, 194
     interfacing with, 114, 116              block scope variables, 175
     packaging with                          blocking, 300
       benefits of, 77                       blocks, 348
       configure.ac shell script, 87         Boolean arguments, 183, 363
       content variables, 83                 Bourne shell, 15
       example run, 79                       BSD (Berkeley Software Distribution), 16, 363
       flowchart for, 81                     BSD License, 17
       form variables, 82
       makefile bits, 86                     C
       testing, 85                           C language
     using libraries from source with, 22     basic packages for development, 2
                                              Boolean arguments and, 183
B                                             comparing unsigned integers, 156
backtraces, 33, 36, 49                        concurrent C code (see parallel threads)
basic environment setup                       const keyword
     compiling via here document              as type qualifier, 177
       benefits of, 27                        char const** issue, 180
       compilation pattern, 25                depth of, 179
       compiling from stdin, 28               noun-adjective form of, 178
       including header files, 25             tension created by, 179
       unified headers, 26                    declarations in, 142
     compiling with Windows                   double vs. float, 153
       POSIX for Windows, 4                   easier text handling in (see text handling)
       with POSIX, 6                          enums and strings, 147
       without POSIX, 7                       explicitly returning from main, 141
     libraries                                externally linked variables in header files,
       compiler flags, 10                     175
       compiler warnings, 11                  goto function, 149
       generating shared, 21                  inessential concepts in, 141
       locating, 12                           linkage with static and extern, 174
       paths, 11                              macros in
       private root directory for, 23         arguments and, 169
       runtime linking, 14                    basic principle, 165
       using, 8                               commenting out code, 173
       using from source, 22                  commonly defined, 170
     makefiles                                cultivating robust, 161
       benefits of, 15                        debugging, 165
       built-in variables, 18                 do-while loops, 164
       environment variables, 17              header guards, 172
       rules for, 18                          preprocessor tricks, 166
       setting variables, 15                  syntax rules, 162
     overview of, 1                           test macros, 170
     package manager, 2                       text substitutions with, 162
                                              types of, 162
...
## Page 399
    ...
    object-oriented programming in (see     rules for, 18
    object-oriented programming (OOP))      setting variables, 15
    parsing strings to numbers, 156         Python example, 116
    restrict vs. inline keywords, 178       setup for
    setting array size at runtime, 144      overview of, 1
    switch function, 152                    package managers/packages, 2
    syntax for variables, 15                via here document
    tutorial                                benefits of, 27
    functions and expressions, 352          compilation pattern, 25
    language structure, 343                 compiling via stdin, 28
    pointers, 358                           including header files, 25
    variable declarations, 346              unified headers, 26
    type-casting syntax in, 146             with Windows
    useful concepts in, 161                 using POSIX, 4
    UTF encoding for, 199                   without POSIX, 7
    void pointer in malloc, 145            compiler flags, 114
    while keyword, 148                     complex numbers, 14, 271
    C standards                            compound data types, 221
    ANSI C89, 13                           compound literals, 206
    ISO C11, 14                            concurrent programming (see parallel threads)
    ISO C99, 14                            conditional subdirectories, 116
    K & R (ca. 1978), 13                   configure.ac shell script, 87
    C threads, 312                         const keyword
    C11 standard, 14                        as type qualifier, 177
    C99 standard, 14                        char const** issue, 180
    call graphs, 60, 363                    depth of, 179
    call stack, 33                          noun-adjective form of, 178
    callback function, 234, 363             tension created by, 179
    casting, 145                           constant strings, 189
    char const **, 180                     content variables, 83
    check_name function, 181               controlling expression, 273
    Church, Alonso, 247                    copy command, 129
    commit objects, 95                     copyright laws, 16
    compilation                            critical regions, 291, 301, 319
    basic process, 344                     cURL library, 335
    compiler flags, 10, 110, 114           CWEB documentation system, 61
    libraries                              Cygwin, 5, 8
    compiler warnings, 11
    generating shared, 21                  D
    locating, 12                           data structures
    paths, 11                               bridging across languages, 112
    private root directory for, 23          generic, 239
    runtime linking, 14                     memory management and, 131
    using, 8                                non-public, 112
    using from source, 22                  debugging/testing
    linkage advice in, 174                  compiler flag for, 32
    makefiles                               of Autotools packaging, 85
    benefits of, 15                         profiling, 48
    built-in variables, 18                  unit testing
...
## Page 400
       ...
       test coverage, 54                            Extensible Markup Language (XML), 335
       test harnesses for, 50                       extern keyword, 174, 175
       using programs as libraries, 53              external linkage, 174
     using a debugger                               external pointers, 113, 364
       choices of, 32
       common debugger commands, 40                 F
       example of, 34                               fall-through, 152
       GDB variables, 43                            fc command, 73
       importance of, 31                            files
       printing structures from, 44                  header files, 345
       text editor for, 33                           operating on sets of, 68
     using Valgrind, 48                              retrieving text files, 107
decimal points, 153                                  scope variables for, 174
declarations, 142, 178, 346                          shared object files, 107
deep copy, 129, 131, 364                             testing for, 70
designated initializers, 133, 155, 205, 214, 257     version control of, 94
dictionaries                                        flags
     extending, 247                                  compiler, 10, 110
     implementing, 249                               linker, 21
diff command, 94                                    float function, 153
distributed revision control systems, 94            floating-point math, 153, 158, 364
Distutils, 118                                      for loops, 69
dlopen function, 107                                foreach loops, 212
dlsym function, 107                                 form variables, 82
do-while loops, 164                                 frames
documentation                                        debugging with, 36
     importance of, 59                               definition of, 364
     using CWEB, 61                                  generating, 33
     using Doxygen, 60                               variable handling in, 125
Doxygen, 60                                         free function, 212
dynamic loading, 107                                Friendly Interactive shell (fish), 66
                                                    functions
E                                                    callback functions, 234
echo command, 70                                     compound literal values and, 206
egg replacer recipe, 261                             declaring, 175, 347
enumeration, 148                                     evaluation of, 353
enums, 147                                           hash functions, 263
environment variables, 17, 67, 364                   retrieving/using, 108
erf function, 9                                      returning multiple items from, 220
error checking                                       static keyword and, 175
     approach to, 55                                 structured inputs for, 205
     indication return, 59, 221                      type-generic, 271
     user's context, 57                              variadic, 223
     user's involvement, 56                          vectorized, 212
exit function, 150
expansions, 67, 162                                 G
expressions, 353                                    GDB (GNU Debugger)
extended regular expressions (EREs), 323             common commands in, 40
extensibility, 248                                   example run of, 34
...
## Page 401
       ...
       printing elements from, 36                   H
       usability of, 32                             hash functions, 263
       variables for, 43                            headers
getenv, 17                                           externally linked variables in, 175
getopt function, 152                                   header files, 345
GetProcAddress function, 107                         header guards in macros, 172
Gettext, 203                                         including header files, 25
Git                                                  public, 27
       adding elements to list of changes, 96        unified, 26
       amending commits, 97                         heap, definition of, 364
       central repository, 105                      Hello, World script, 79
       changes via diff command, 94                 here documents
       cloning a repository, 95                      compilation via
       commit messages, 97                           benefits of, 27
       commit objects, 95                            compilation pattern, 25
       commits in, 97                                compiling from stdin, 28
       creating a repository, 95                     including header files, 25
       git diff command, 98                          unified headers, 26
       git fetch command, 104
       git log command, 98                          I
       git push origin master command, 104          iconv function, 200
       git reflog command, 98
       graphical interfaces for, 101                if statements, 70
       HEADS in, 99                                 include guards, 173
       index in, 96                                 INFINITY, 158, 170
       merging changes in, 101                      inline keyword, 178
       rebase command, 103                          Institute of Electrical and Electronics Engineers
       remote repositories, 104                      (IEEE), 15
       snapshots, 97                                integers, signed vs. unsigned, 156
       stash object, 99                             integration tests, 50, 364
       trees and branches in, 100                   interfaces
GLib                                                 dynamic loading and, 107
       gmodules, 110                                 process of writing
       samples of, 321                                   bridging data structures, 112
global variables, 364                                linking, 114
glyphs, 364                                              to be read by nonnatives, 111
GNU Autoconf macro archive, 90                       using dlopen/dlsym, 110
GNU C, 16                                            wrapper function, 111
GNU C library, manual for, 24                        Python example
GNU Gettext, 203                                     Automake conditional subdirectory, 116
GNU Libiberty library, 185                           compiling/linking, 116
GNU Public License (GPL), 16                         controlling package production, 118
GNU Readline library, 110                            overview of, 114
GNU Scientific Library (GSL), 13, 22, 130, 139,     INTERIX, 5
       330                                          internal linkage, 174
GNU Screen, 72                                      International Organization for Standardization
go-to-the-exit functions, 150                        (ISO), 13
goto function, 149                                  internet protocols, 335
Graphviz, 60                                        interoperability, 65
                                                    ISO C11 standard, 14
...
ISO C99 standard, 14                                 multiple, 210
ISO/IEC 8859 format, 198                             returning multiple values with, 220
                                                     safely terminated, 209
K                                                    vs. block-of-memory, 248
K & R standard, 13                                  literate programming, 61
Kernighan, Brian, 13                                LLDB Debugger
keyboarding skills, 63                               common commands in, 40
                                                     popularity of, 32
L                                                    printing structures from, 47
lambda calculus, 247                                 sample run of, 34
Lesser GPL (LGPL), 17                               LoadLibrary function, 107
libiconv function, 200                              long double function, 153
libraries                                           M
     commonly used C libraries, 4
     compiler flags, 10                             macros
     compiler warnings, 11                           # (octothorps) and, 166
     cURL, 335                                       arguments and, 169
     definition of, 364                              basic principle of, 165
     distribution licenses, 16                       commenting out code, 173
     dynamic loading of, 107                         commonly defined, 170
     generating shared, 21                           cultivating robust, 161
     GLib, 321                                       debugging, 165
     GNU C, 24                                       definition of, 364
     GNU Scientific Library (GSL), 330               do-while loops in, 164
     improvements in, 321                            header guards, 172
     libxml, 335                                     preprocessor tricks, 166
     locating, 12                                    SQL query, 334
     paths, 11                                       syntax rules, 162
     POSIX                                           test macros, 170
           gigantic data sets and mmap, 327          text substitutions with, 162
           parsing regular expressions with, 322     types of, 162
     private root directory for, 23                  variable-length arguments, 208
     runtime linking, 14                            main function, 33, 141, 352
     standard, 345                                  makefiles
     static vs. shared, 14                           benefits of, 15
     Structured Query Language (SQL), 332            built-in variables, 18
     typical format for, 245                         environment variables, 17
     using, 8                                        Makefile.am
     using from source, 22                                content variables, 83
Libtool, 21, 79, 86                                       form variables, 82
     (see also Autoconf; Automake)                        makefile bits, 86
Libxml                                               rules for, 18
     tagged trees in, 335                            setting variables, 15
     UTF-8 encoding in, 200                          vs. shell scripts for packaging, 75
licenses, 16                                        malloc
linker flags, 21                                     avoiding bugs related to, 134
linking, 114, 116, 174                               location of memory allocated via, 125
list command, 34                                     reasons to use, 134
lists                                                void pointer in, 145
...
manual memory management, 124                  extending, 247
math, floating point, 153                      implementing a dictionary, 249
memory leaks, 50, 191                          inserting new elements, 253
memory management                              typical library format, 245
     asprintf vs. sprintf functions, 188       variable scope in C, 267
     automatic, 124                            virtual tables (vtables)
     basic models of, 123                      adding new functions with, 262
     block-of-memory approach, 247             benefits of, 267
     disk-based memory, 327                    hash function, 263
     heaps and, 125                            macro for, 265
     manual, 124, 364                          type checking, 264
     persistent state variables, 128           _Generic keyword, 271
     stack size limits, 125                   octothorp (#), 166
     static, 124                              omp_set_lock function, 302
     unsigned integers in, 156                once pragma, 173
     vs. variable management, 126             opaque pointers, 113, 365
     _Thread_local memory type, 129           OpenMP
memory twiddling, 134                          compiling, 294
MinGW (Minimalist GNU for Windows), 7          interference, 295
multibyte encoding, 201, 365                   map-reduce, 296
multidimensional arrays, 138                   multiple tasks, 297
multiplexers, 72                               word-counting program example, 292
multithreading                                operator overload, 270
     debugging, 37                            overload, 270
     environments for, 290
     (see also parallel threads)              P
mutex (see mutual exclusion)                  package managers, 2
mutual exclusions (mutexes), 302, 365         packages, basic set for C development, 2
                                              packaging
N                                              interoperability principle, 65
narrative pages, 61                            makefiles vs. shell scripts, 75
non-const pointers, 179                        shell management
Not-a-Number (NaN), 158, 365                   fc command, 73
noun-adjective declarations, 178               POSIX-standard shell features, 66
numbers, parsing from strings, 156             replacing commands with outputs, 67
numeric drift, 153                             testing for files, 70
                                               using shell loops, 68
O                                              with Autotools
object-oriented programming (OOP)              benefits of, 77
     common features of, 246                   configure.ac shell script, 87
     count references                          content variables, 83
     adding a reference counter, 274           example run, 79
     agent-based model of group formation,     flowchart for, 81
     279                                       form variables, 82
     substring object example, 275             makefile bits, 86
     element-of-struct form, 258               testing, 85
     operator overload, 270                   parallel threads
     pointer pros/cons, 257                    atomic structs, 315
     structures/dictionaries                   C atoms, 312
...
## Page 404
    ...
    listing, 37                              definition of, 365
    localizing nonstatic variables, 300      GNU implementation, 16
    mechanism for, 290                       history of standard, 15
    OpenMP                                   mmap for gigantic data sets, 327
      compiling, 294                         parsing regular expressions with, 322
      interference, 295                      standard naming conventions, 218
      map-reduce, 296                        standard shell features, 66
      multiple tasks, 297                   POSIX versions
      word-counting program example, 292     BSD, 16
    overview of creating, 289                GNU, 16
    pthread_create function, 308             POSIX, 15
    sequential consistency and, 307          Unix, 15
    shared resources in                     preprocessor
      atoms, 305                             comment out code with, 173
      blocking, 300                          definition of, 365
      mutual exclusions (mutexes), 302       handling diverse environments with, 170
      parallelization of, 302                macro processing, 166
    thread local, 298                        purpose of, 345
    threadprivate clause, 298               printf function, 346
parsing, strings to numbers, 156            profiling, 48, 365
patch command, 94                           pthread, 365
Perl, regular expressions in, 323           pvnrt package, 114
persistent state variables, 128             Python
pkg-config, 13                               interfacing with
plugin systems, 108                          compiling/linking, 116
pointers                                     conditional subdirectory for Automake,
    basic function of, 358                       116
    definition of, 123                       controlling package production, 118
    in language interfaces, 112              overview of, 114
    manual vs. automatic memory and, 126
    memory types and features, 123          Q
    models of memory management, 123        quick_exit function, 150
    non-const, 179
    persistent state variables and, 128     R
    pros/cons of, 257                       random number generator (RNG), 330, 365
    returning multiple items with, 221      Readline library, 110
    variable management and, 126
    void, 145, 234, 239                     regular expressions
    vs. arrays, 127                          benefits of, 322
    without malloc                           POSIX/PCRE interfaces, 323
      array notation, 136                    types of, 323
      copies and aliases, 129               resources, sharing, 300
      declaration syntax, 135               restrict keyword, 178
      memory-twiddling, 134                 revision control systems (RCSes), 93
      structures vs. arrays, 131            Ritchie, Dennis, 13
      typedef and, 139                      runtime linking, 114
POSIX
    BSD implementation, 16                  S
    compiling C with, 4                     Sapir-Whorf hypothesis, 246, 365
...
## Page 405
    ...
    Sasprintf macro, 191                        strtok function
    scope, 267, 365                              basic working of, 193
    segfaults, 124, 224, 365                     tasks possible with, 192
    seq command, 70                             strtok_r function, 193
    sequential consistency, 307                 strtok_s function, 194
    shared libraries, 14, 21                    strtol command, 157
    shared object files, 107                    strtoll command, 113
    shell management                            structs
    choosing shells, 73                          compound literals and, 216
    fc command, 73                               declaring format of, 217
    replacing commands with outputs, 67          designated initializers and, 205
    testing for files, 70                        error codes and, 244
    using shell loops, 68                        filling, 215
    shell scripts                                function-specific, 220
    configure.ac, 87                             initializing with zeros, 216
    fc command, 73                               sending to functions, 206
    vs. makefiles for packaging, 75              specifying, 214
    signed integers, 156                         writing new, 221
    simple command description, 17              structured inputs
    sizeof function, 156, 167, 350               array/struct initialization with zeros, 216
    snprintf function, 349                       compound literals, 206
    source shell command, 68                     designated initializers, 214
    stacks                                       error code reporting, 221
    arbitrary size limits in, 125                for variadic functions
    definition of, 366                                 improving interface for, 228
    state variables, 319                               lack of type safety in, 223
    static allocation, 366                             optional and named arguments, 226
    static keyword, 174                                printf-style declaration, 224
    static libraries, 14                         foreach loops, 212
    static memory management, 124                multiple lists, 210
    static variable, 128, 298                    returning multiple items, 220
    stdn, compiling from, 28                     safely terminated lists, 209
    strdup function, 190                         typedef and, 217
    string types, 351                            variable-length macro arguments, 208
    strings, 147                                 vectorized functions, 212
    asprintf function                            void pointer
              constant strings, 189                    for generic inputs, 234
              extending strings with, 191              generic structures, 239
              sensitive information and, 188     working with, 205
              string allocation with, 185       Structured Query Language (SQL), 332
    creating from macros, 166                   structures, extending, 247
    drawbacks of C, 185                         subdirectories
    duplicating, 190                             anonymous, 254
    parsing to numbers, 156                      conditional, 116
    string-handling utilities, 195              Subsystem for Unix-based Applications (SUA),
    strtok function                              5
              basic working of, 193             switch function, 152
              tasks possible with, 192          system command, 110
    strtod command, 157
...
T                                              encoding for C code, 199
tab completion feature, 66                     libraries for, 200
technical documentation, 61                    premise of, 197
tentative definitions, 176                     programming caveats, 198
terminal multiplexers, 72                      sample program, 202
test harnesses, 50, 366                        UTF encoding, 197
TeX documentation system, 61                  union keyword, 256
text files, retrieving, 107                   unit testing
text handling                                  test coverage, 54
     asprintf function                         test harnesses, 50
           constant strings, 189               using programs as libraries, 53
           extending strings with, 191        Unix standard, 15
           sensitive information and, 188     unsigned integers, 156
           string allocation with, 185        UTF encoding, 197, 199
     drawbacks of C for, 185
     string-handling utilities, 195           V
     strtok function                          Valgrind, 48
           basic working of, 193              variables
           tasks possible with, 192            atomic, 312
     translations, 203                         automatic management of, 124
     Unicode                                   block scope, 175
           display of, 198                     built-in, 18
           encoding for C code, 199            content variables, 83
           premise of, 197                     convenience variables, 43
           programming caveats, 198            creating auxilary with preprocessor, 167
           sample program, 202                 declarations, 346
           Unicode libraries, 200              environment, 17, 67, 364
           UTF encoding, 197                   externally linked in header files, 175
threads (see parallel threads)                 file scope variables, 174
tmux multiplexer, 72                           for GDB (GNU Debugger), 43
tokenization, 192, 366                         form variables, 82
translations, 203                              global, 364
tuples, 221                                    in macros, 163
Turing, Alan, 247                              in memory management, 126
two-dimensional arrays, 138                    incrementing/scaling, 354
type checking, 264                             localizing nonstatic, 300
type punning, 366                              private copies of, 298
type qualifier, 177, 366                       scope, 267, 352
type-casting, 146                              setting, 15
typedef, 139, 172, 217                         shell variables, 67
types                                          state, 319
     aggregating, 348                          static, 124, 128, 298
     defining, 349                             undefined, 216
     determining size of, 350                 variadic functions, 223
                                              variadic macros, 208
U                                             version control, 93
U. S. copyright laws, 16                      virtual tables (vtables)
Unicode                                        adding new functions with, 262
     display of, 198                           benefits of, 267
...
## Page 407
     ...
     hash function, 263                 compilation with, 4-8
     macro for, 265                     UTF encoding in, 198, 201
     type checking, 264                wrapper function, 111
Visual Studio, 6
vsnprintf function, 185                X
W                                      XML libraries, 245
while keyword, 148                     Z
whitespace, preprocessors and, 166     Z shell, 73
...
- Local path: `[[books/21st Century C, 2nd Edition.pdf]]`
