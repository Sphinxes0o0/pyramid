...
type: source
source-type: web
title: "Tip of the Week #108: Avoid `std::bind`"
author: "Roman Perepelitsa"
date: 2012
url: https://abseil.io/tips/108
summary: "abseil C++ Tips of the Week #108: Avoid `std::bind."
tags: [cpp, abseil, totw, performance, idioms]
created: 2026-06-10
source-md5: 8b8e95dfaf6df380def0b330cf18ee51
---

> 原文：<https://abseil.io/tips/108> · 作者：Roman Perepelitsa · 发布：2012 · 内容许可：CC BY 4.0（abseil.io docs）
>
...
*By [Roman Perepelitsa](/cdn-cgi/l/email-protection#84f6ebe9e5eaaaf4e1f6e1f4e1e8edf0f7e5c4e3e9e5ede8aae7ebe9)*
...
**`std::bind()` doesn’t just bind the first N arguments** like many C++ engineers expect (this is called [partial function application](https://en.wikipedia.org/wiki/Partial_application)). You must instead specify every argument, so the correct incantation of `std::bind()` is this:
...
Ugh, that’s ugly. Is there a better way? Why yes, use [`std::bind_front()`](https://en.cppreference.com/w/cpp/utility/functional/bind_front) instead. (For code that cannot yet use C++20, there’s `absl::bind_front`.)
